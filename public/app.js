let map, clusterer, debugPanel;
let allApts = [];
let selectedA = null;
let selectedB = null;
let areas = {};
let clusterLayerVisible = false;
...
async function loadKakaoMap() {
  ...
  await new Promise((resolve, reject) => {
    ...
    script.src = `https://dapi.kakao.com/v2/maps/sdk.js?appkey=${cfg.kakaoJsKey}&autoload=false&libraries=services,clusterer`;
...
function initMap() {
  map = new kakao.maps.Map($('map'), {
    center: new kakao.maps.LatLng(37.49, 126.99),
    level: 10
  });
...
function bindEvents() {
  ...
  $('q').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') loadApartments();
  });
  $('sido').onchange = () => fillSigungu();
  document.querySelectorAll('[data-chip]').forEach((btn) => {
    btn.onclick = () => {
      document.querySelectorAll('[data-chip]').forEach((b) => b.classList.remove('active'));
      if (currentFilters.chip === btn.dataset.chip) {
        currentFilters.chip = null;
      } else {
        currentFilters.chip = btn.dataset.chip;
...
function fillAreas() {
  $('sido').innerHTML = '<option value="">수도권 전체</option>' +
    Object.keys(areas).map((s) => `<option>${s}</option>`).join('');
  fillSigungu();
}

function fillSigungu() {
  const sido = $('sido').value;
  $('sigungu').innerHTML = '<option value="">전체 시군구</option>' +
    (areas[sido] || []).map((g) => `<option>${g}</option>`).join('');
}
...
function transparentMarkerImage() {
  ...
  return new kakao.maps.MarkerImage(src, new kakao.maps.Size(1, 1), {
...
function fillCompareSelects() {
  ...
  const opts = Array.from(seen.values()).map((a) => `<option value="${a.id}">${a.name} (${a.sido} ${a.sigungu})</option>`).join('');
  $('cmpA').innerHTML = opts;
  $('cmpB').innerHTML = opts;
